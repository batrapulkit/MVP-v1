...
// Enhanced HeroSection Component with Reference UI Styling
...
const HeroSection = ({ onStartPlanning }: HeroSectionProps) => {
  ...
  const ABSTRACT_ROTATE_X = 25;
  const ABSTRACT_ROTATE_Y = 25;
...
import { Sparkles, MoveRight, Play, Globe, MapPin, Rocket, Cloud, ChevronDown } from 'lucide-react';
...
const Home = () => {
  ...
  const onStartPlanning = () => {
    setChatOpen(true);  // open ChatBot
  };

  const onCloseChat = () => {
    setChatOpen(false); // close ChatBot
  };
  ...
  const handlePreferenceComplete = (savedPreference: TravelPreference) => {
    ...
    const message = `Hi! I want to travel to ${
      savedPreference.customDestination
    } from ${savedPreference.startDate} to ${
      savedPreference.endDate
    }. My budget is ${savedPreference.budget}. Interests: ${
      savedPreference.interests || "general travel"
    }.`;

    setChatInput(message);   // Set the initial input for ChatBot
    setChatOpen(true);       // Open the ChatBot UI
  };
