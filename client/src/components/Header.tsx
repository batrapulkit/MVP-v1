...
import { fetchCurrentUser, logoutUser } from "@/api/services";
import {
  Menu,
  X,
  Plane,
...
// Adjusted User type to match your API response
type User = {
  user_id: string;
  email: string;
  role_type: string;
    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          {/* User Profile or Sign Up & Sign In Buttons */}
          <div className="hidden md:flex items-center gap-3 ml-6">
            {user ? (
              <div className="relative" ref={profileDropdownRef}>
                <button
                  onClick={() => setProfileOpen(!profileOpen)}
                  className="flex items-center gap-2 px-3 py-2 rounded-full hover:bg-cyan-900/20 transition text-white"
                >
                  <span className="inline-block w-9 h-9 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 flex items-center justify-center font-bold text-white text-sm">
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-cyan-900/20 border border-cyan-400 rounded-xl shadow-lg z-50">
                    <Link href="/profile">
                      <a className="block px-4 py-2 text-sm text-white hover:text-cyan-400 hover:bg-cyan-900/20 rounded-t-xl transition">Profile</a>
                    </Link>
                    <button
                      onClick={handleLogout}
                      className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-cyan-900/20 rounded-b-xl transition"
                    >
                      Log out
                    </button>
                  </div>
